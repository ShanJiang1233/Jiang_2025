...
channelOption='selected'; % Available inputs: 'all' or 'selected'
channels= [4]; % Channel indices. Only used in 'selected' mode.
...
thresholdMode='SetValue'; % Available inputs: 'SetValue' or 'Median' 
mechNoiseSuppress='no'; % For single-batch data analysis without mech noise
thresholdSetValue = 25; % in uV. Only used in 'SetValue' mode.
artifactCutoff = 2000; % Maximum amplitude for artifact removal, in uV.
...
%originalData=read_Intan_RHD2000_file_no_prompt(fname,pname); % rows: channels; columns: times
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CHANNEL REORDERING %%%%%%%%%%%%%%%%%%%%%%%%%%
% dataBuffer=originalData;
% originalData=[];
% originalData(1,:)=dataBuffer(2,:);
% originalData(2,:)=dataBuffer(1,:);
% originalData(3,:)=dataBuffer(3,:);
% originalData(4,:)=dataBuffer(4,:);
% originalData(5,:)=dataBuffer(6,:);
% originalData(6,:)=dataBuffer(5,:);
% originalData(7,:)=dataBuffer(7,:);
% originalData(8,:)=dataBuffer(8,:);
% originalData(9,:)=dataBuffer(9,:);
% originalData(10,:)=dataBuffer(10,:);
% originalData(11,:)=dataBuffer(25,:);
% originalData(12,:)=dataBuffer(12,:);
% originalData(13,:)=dataBuffer(13,:);
% originalData(14,:)=dataBuffer(14,:);
% originalData(15,:)=dataBuffer(15,:);
% originalData(16,:)=dataBuffer(16,:);
% originalData(1,:)=dataBuffer(1,:);
% originalData(2,:)=dataBuffer(2,:);
% originalData(3,:)=dataBuffer(17,:);
% originalData(4,:)=dataBuffer(18,:);
% originalData(5,:)=dataBuffer(19,:);
% originalData(6,:)=dataBuffer(20,:);
% originalData(7,:)=dataBuffer(21,:);
% originalData(8,:)=dataBuffer(22,:);
% originalData(9,:)=dataBuffer(23,:);
% originalData(10,:)=dataBuffer(24,:);
% originalData(11,:)=dataBuffer(26,:);
% originalData(12,:)=dataBuffer(27,:);
% originalData(13,:)=dataBuffer(28,:);
% originalData(14,:)=dataBuffer(31,:);
% originalData(15,:)=dataBuffer(32,:);
...
% end
iterNum=1;
filteredData = filtfilt(butterB,butterA, transposedData);
for i=1:iterNum
filteredData = filtfilt(butterB,butterA, filteredData);
end
